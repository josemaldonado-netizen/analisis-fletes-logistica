...
def _normalizar_mes(valor):
    """Convierte números y nombres de mes a la misma etiqueta canónica."""
    if pd.isna(valor):
        return np.nan
    ...
    if pd.notna(numero) and float(numero).is_integer() and 1 <= int(numero) <= 12:
        return NOMBRES_MESES[int(numero) - 1]
    return texto
...
def procesar_archivo_cached(file_bytes: bytes, file_name: str):
    ...
    if not col_mes_fact:
        ...
        # Normaliza nombres, abreviaturas y números de mes para evitar meses invisibles.
...
def _etiqueta_periodos(seleccionados):
    """Etiqueta legible para uno o varios periodos seleccionados."""
    if not seleccionados:
        return "Sin selección"
    return ", ".join(str(valor) for valor in seleccionados)
...
if modo_periodo == "Semana":
    ...
    if col_periodo not in df.columns or df[col_periodo].dropna().empty:
        ...
        periodos_a = st.sidebar.multiselect(
            "Semanas A (Base)", periodos, default=periodos[:1], key="semanas_base"
        )
        periodos_b = st.sidebar.multiselect(
            "Semanas B (Actual)", periodos,
            default=periodos[1:2] or periodos[:1], key="semanas_actual"
        )
        if not periodos_a or not periodos_b:
            st.sidebar.error("Selecciona al menos un periodo para cada grupo.")
            datos_validos = False; per_a = per_b = None
        else:
            df_a = df[df[col_periodo].isin(periodos_a)]
            df_b = df[df[col_periodo].isin(periodos_b)]
            df_periodos_seleccionados = df[df[col_periodo].isin(set(periodos_a) | set(periodos_b))]
            per_a = _etiqueta_periodos(periodos_a)
            per_b = _etiqueta_periodos(periodos_b)

elif modo_periodo == "Mes":
    col_periodo='MES FACTURA'
    # Los meses se ofrecen para todos por igual, independientemente del filtro operativo.
    # El cálculo posterior conserva únicamente los registros disponibles para cada grupo.
    meses_disponibles = NOMBRES_MESES
    if col_periodo not in df.columns:
        st.sidebar.error("No hay datos de MES FACTURA.")
        datos_validos=False; per_a=per_b=None
    else:
        meses_con_datos = [mes for mes in meses_disponibles if (df[col_periodo] == mes).any()]
        default_a = meses_con_datos[:1] or meses_disponibles[:1]
        default_b = meses_con_datos[1:2] or meses_con_datos[:1] or meses_disponibles[:1]
        periodos_a = st.sidebar.multiselect(
            "Meses A (Base)", meses_disponibles, default=default_a, key="meses_base"
        )
        periodos_b = st.sidebar.multiselect(
            "Meses B (Actual)", meses_disponibles, default=default_b, key="meses_actual"
        )
        if not periodos_a or not periodos_b:
            st.sidebar.error("Selecciona al menos un periodo para cada grupo.")
            datos_validos = False; per_a = per_b = None
    ...
    # Día: permite seleccionar cualquier cantidad de fechas individuales.
    if 'FECHA_FACTURA_DT' not in df.columns or df['FECHA_FACTURA_DT'].dropna().empty:
        ...
        periodos = sorted(df['FECHA_FACTURA_DT'].dropna().dt.date.unique())
        periodos_a = st.sidebar.multiselect(
            "Días A (Base)", periodos, default=periodos[:1], key="dias_base"
        )
        periodos_b = st.sidebar.multiselect(
            "Días B (Actual)", periodos,
            default=periodos[1:2] or periodos[:1], key="dias_actual"
        )
        if not periodos_a or not periodos_b:
            st.sidebar.error("Selecciona al menos un periodo para cada grupo.")
            datos_validos = False; per_a = per_b = None
        else:
            fechas_a = pd.to_datetime(periodos_a).normalize()
            fechas_b = pd.to_datetime(periodos_b).normalize()
            df_a = df[df['FECHA_FACTURA_DT'].isin(fechas_a)]
            df_b = df[df['FECHA_FACTURA_DT'].isin(fechas_b)]
            df_periodos_seleccionados = df[df['FECHA_FACTURA_DT'].isin(fechas_a.union(fechas_b))]
            per_a = _etiqueta_periodos([fecha.strftime('%Y-%m-%d') for fecha in periodos_a])
            per_b = _etiqueta_periodos([fecha.strftime('%Y-%m-%d') for fecha in periodos_b])
...
with tab1:
    ...
    # La tendencia respeta todos los periodos seleccionados en ambos grupos.
    df_trend = df_periodos_seleccionados.copy()
    # Agrupación robusta
