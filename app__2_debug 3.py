...
# Equivalencias de nombres de mes en inglés (y abreviaturas comunes de Excel)
# hacia el nombre en español usado internamente por la app.
MESES_EN_A_ES = {
...
def _normalizar_nombre_mes(nombre) -> str:
    """
    Convierte un nombre de hoja/mes (español, inglés, con espacios extra,
    acentos o mayúsculas/minúsculas mezcladas) al nombre en español que usa
    internamente la app. Si no reconoce el nombre, lo regresa normalizado
    tal cual (upper + sin espacios) para que el llamador decida qué hacer.
    """
    n = str(nombre).strip().upper()
    n = ''.join(c for c in unicodedata.normalize('NFD', n) if unicodedata.category(c) != 'Mn')
    if n in NOMBRES_MESES:
        return n
    if n in MESES_EN_A_ES:
        return MESES_EN_A_ES[n]
    return n
...
def _detectar_dayfirst(serie_fechas: pd.Series) -> tuple:
    """
    Detecta si el formato de fecha en texto es DD/MM/YYYY (día primero, típico
    en español/México) o MM/DD/YYYY (mes primero, típico en Excel configurado
    en inglés/US), mirando si algún componente numérico excede 12.

    Retorna (dayfirst: bool, es_ambiguo: bool).
    Si la columna ya es datetime (Excel guardó fecha real, no texto), no aplica
    y se debe usar directamente sin heurística.
    """
    muestra = serie_fechas.dropna().astype(str).head(500)
    primer_comp = pd.to_numeric(muestra.str.extract(r'^(\d{1,4})')[0], errors='coerce').dropna()
    segundo_comp = pd.to_numeric(muestra.str.extract(r'^\d{1,4}[/\-.](\d{1,2})')[0], errors='coerce').dropna()
    if (primer_comp > 12).any():
        # El primer número no puede ser mes -> es día -> formato día-primero
        return True, False
    if (segundo_comp > 12).any():
        # El segundo número no puede ser mes -> es día -> formato mes-primero
        return False, False
    # Todos los días detectados son <=12: no se puede distinguir con certeza.
    # Se asume mes-primero (MM/DD, típico de Excel en inglés) pero se marca ambiguo.
    return False, True
...
def procesar_archivo_cached(file_bytes: bytes, file_name: str):
    ...
    if file_name.lower().endswith('.xlsx'):
        ...
        # Mapa: nombre_normalizado (ES, sin importar si la hoja venía en inglés) -> nombre real de la hoja
        mapa_hojas_normalizadas = {_normalizar_nombre_mes(h): h for h in excel_file.sheet_names}
        # Lee solo hojas que son meses (en español o inglés); si ninguna coincide, lee todas las hojas con datos
        hojas_a_leer = [mapa_hojas_normalizadas[m] for m in NOMBRES_MESES if m in mapa_hojas_normalizadas]
        if not hojas_a_leer:
            hojas_a_leer = excel_file.sheet_names
        else:
            hojas_ignoradas = [h for h in excel_file.sheet_names if h not in hojas_a_leer]
        for nombre_hoja in hojas_a_leer:
            ...
            # Asigna MES_ORIGEN normalizando el nombre de hoja (soporta inglés/acentos/espacios)
            nombre_mes_normalizado = _normalizar_nombre_mes(nombre_hoja)
            if nombre_mes_normalizado in NOMBRES_MESES:
                df_temp['MES_ORIGEN'] = nombre_mes_normalizado
            else:
    ...
    meta = {}
    if hojas_ignoradas:
        meta['hojas_ignoradas'] = hojas_ignoradas
    ...
    if col_ff:
        if pd.api.types.is_datetime64_any_dtype(df_out[col_ff]):
            # Excel ya entrega tipo fecha real: no hay ambigüedad de formato de texto.
            df_out['FECHA_FACTURA_DT'] = pd.to_datetime(df_out[col_ff], errors='coerce').dt.normalize()
        else:
            dayfirst_detectado, es_ambiguo = _detectar_dayfirst(df_out[col_ff])
            df_out['FECHA_FACTURA_DT'] = pd.to_datetime(
                df_out[col_ff], errors='coerce', dayfirst=dayfirst_detectado
            ).dt.normalize()
            meta['formato_fecha_detectado'] = 'DD/MM/AAAA (día primero)' if dayfirst_detectado else 'MM/DD/AAAA (mes primero)'
            if es_ambiguo:
                meta['formato_fecha_ambiguo'] = True
        # Alerta de calidad: si muchas fechas no se pudieron parsear, algo sigue mal
        total_fechas = df_out[col_ff].notna().sum()
        fechas_perdidas = int(df_out['FECHA_FACTURA_DT'].isna().sum() - df_out[col_ff].isna().sum())
        if total_fechas > 0 and fechas_perdidas / total_fechas > 0.15:
            meta['alerta_fechas_no_parseadas'] = (
                f"{fechas_perdidas} de {total_fechas} fechas en 'FECHA FACTURA' no se pudieron "
                f"interpretar y quedaron vacías. Revisa el formato de esa columna."
            )
    else:
    ...
    if not col_mes_fact:
        ...
        # normaliza usando el mismo mapa ES/EN que las hojas (soporta 'January', 'Feb', etc.)
        df_out['MES FACTURA'] = df_out[col_mes_fact].apply(_normalizar_nombre_mes)
        df_out.loc[~df_out['MES FACTURA'].isin(NOMBRES_MESES), 'MES FACTURA'] = df_out[col_mes_fact]
...
# --- DEBUG TEMPORAL: fuerza limpieza de caché en cada carga de archivo ---
# Detecta si el archivo (nombre+tamaño+hash) cambió respecto a la última carga
# de ESTA sesión, y si es así, limpia st.cache_data antes de reprocesar.
_bytes_archivo = archivo_subido.getvalue()
_firma_archivo = f"{archivo_subido.name}_{len(_bytes_archivo)}_{hash(_bytes_archivo)}"
_cache_recien_limpiada = st.session_state.get("_firma_archivo_actual") != _firma_archivo
if _cache_recien_limpiada:
    st.cache_data.clear()
    st.session_state["_firma_archivo_actual"] = _firma_archivo

# Procesamiento con cache
with st.spinner("⏳ Procesando datos del archivo..."):
    df_raw, meta = procesar_archivo_cached(_bytes_archivo, archivo_subido.name)

# --- DEBUG: captura meses en df_raw (SIN filtros) para mostrarlo luego en el expander colapsable ---
_debug_meses_sin_filtro = (
    sorted(df_raw['MES FACTURA'].dropna().unique().tolist())
    if 'MES FACTURA' in df_raw.columns else None
)
...
# --- Alertas de calidad de datos (idioma/formato del Excel) ---
if meta.get('hojas_ignoradas'):
    st.warning(
        f"⚠️ Se ignoraron {len(meta['hojas_ignoradas'])} hoja(s) que no se reconocieron como mes "
        f"válido: {', '.join(meta['hojas_ignoradas'])}. Si esperabas que estos datos se incluyeran, "
        f"revisa el nombre de esas pestañas en el Excel."
    )
if meta.get('formato_fecha_detectado'):
    st.caption(f"📅 Formato de fecha detectado en 'FECHA FACTURA': {meta['formato_fecha_detectado']}")
if meta.get('formato_fecha_ambiguo'):
    st.warning(
        "⚠️ No fue posible determinar con certeza si 'FECHA FACTURA' usa formato día-primero (DD/MM) "
        "o mes-primero (MM/DD) — todos los días detectados eran ≤12. Se asumió mes-primero (MM/DD, "
        "típico de Excel en inglés). Si el resultado se ve raro, verifica el formato real de esa columna."
    )
if meta.get('alerta_fechas_no_parseadas'):
    st.error(f"❌ {meta['alerta_fechas_no_parseadas']}")
...
def multiselect_sidebar(col_name, label):
    ...
    # key único por archivo+columna: evita que el multiselect conserve una
    # selección "pegada" de una carga de archivo anterior en la misma sesión.
    sel = st.sidebar.multiselect(label, opts, default=[], key=f"filtro_{col_name}_{_firma_archivo}")
    return sel, real
...
# --- DEBUG: expander colapsable — solo se abre si alguien necesita revisarlo ---
with st.expander("🔍 Información de diagnóstico (clic para expandir)", expanded=False):
    st.write(
        "Meses en df_raw (sin filtros):",
        _debug_meses_sin_filtro if _debug_meses_sin_filtro is not None else "La columna 'MES FACTURA' no existe en df_raw."
    )
    st.write(
        "Meses en df (con filtros aplicados):",
        sorted(df['MES FACTURA'].dropna().unique().tolist()) if 'MES FACTURA' in df.columns else "N/A"
    )
    st.write("Filtros activos:", {
        "Cliente": clientes_sel, "Transportista": transp_sel, "Tipo Transporte": tipo_trans_sel,
        "Tipo Embarque": embarque_sel, "Origen": origen_sel, "Destino": destino_sel
    })
    st.write("Metadatos de procesamiento del archivo:", meta)
    st.write(
        "Caché:",
        f"Limpiada en esta carga (archivo nuevo: {archivo_subido.name}, {len(_bytes_archivo):,} bytes)"
        if _cache_recien_limpiada else "Reutilizada (mismo archivo que la carga anterior en esta sesión)"
    )
