...
def procesar_archivo(file):
    ...
    # Identificar columna de embarque (Columna AA / EMBARQUE)
    col_emb = 'EMBARQUE' if 'EMBARQUE' in df_out.columns else 'TIPO DE EMBARQUE'
    ...
    # LÓGICA DE IDENTIFICACIÓN ÚNICA DE VIAJES (CONSOLIDACIÓN)
    # Crear un ID único de viaje: si tiene Número de Embarque válido, lo usa; si es 'NA', usa el índice solo si TOTAL FLETE > 0
        ...
        for idx, row in df_out.iterrows():
            ...
            if emb not in ['NA', 'NAN', '', 'NONE', '0']:
                ...
                if flete > 0:
                    ...
                    # Fila sin embarque y con flete $0 -> Factura secundaria o costo $0 no contable como viaje
                    id_viajes.append("IGNORAR_VIAJE")

        df_out['ID_VIAJE_UNICO'] = id_viajes
    else:
        # Respaldo por si no viene columna EMBARQUE
...
if archivo_subido is not None:
    ...
    if not df_raw.empty:
        ...
        if len(periodos) > 0:
            ...
            with tab1:
                st.subheader(f"📊 Comparativa Real por Viajes: {modo_periodo} {per_a} vs {modo_periodo} {per_b}")

                # CÁLCULOS CONSOLIDADOS POR ID_VIAJE_UNICO
                # Excluimos las filas comodín que no representan un viaje independiente
                ...
                # METRICAS CLAVE
                st.markdown("### 📐 Resumen de Indicadores Clave (KPIs Reales)")
                m_col1, m_col2, m_col3, m_col4 = st.columns(4)

                with m_col1:
                    st.metric(
                        label=f"Total Viajes Reales ({modo_periodo} {per_a} ➜ {per_b})",
            ...
            with tab2:
                # Agrupar el periodo B por Embarque / Viaje para auditar
...
Analiza la siguiente variación de fletes e imprevistos financieros (calculado por Embarques/Viajes Consolidados) y genera un reporte ejecutivo.

DATOS COMPARATIVOS ({modo_periodo.upper()} {per_a} vs {modo_periodo.upper()} {per_b}):
- Periodo Base ({modo_periodo} {per_a}): Viajes Reales: {viajes_a} | Gasto Total: ${tot_a:,.2f} | Costo Medio/Viaje: ${media_viaje_a:,.2f} | Costo Medio/Tarima: ${costo_tarima_a:,.2f}
- Periodo Actual ({modo_periodo} {per_b}): Viajes Reales: {viajes_b} | Gasto Total: ${tot_b:,.2f} | Costo Medio/Viaje: ${media_viaje_b:,.2f} | Costo Medio/Tarima: ${costo_tarima_b:,.2f}
...
1. 📌 Resumen Ejecutivo
2. 🚨 Alertas Operativas (Desviación en Costo por Viaje/Tarima)
