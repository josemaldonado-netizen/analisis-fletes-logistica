...
with tab1:
    ...
    if 'ID_VIAJE_UNICO' in df_b.columns and 'TIPO DE TRANSPORTE' in df_b.columns:
        df_eficiencia = df_b[df_b['ES_CUENTA_VIAJE']==True].groupby('ID_VIAJE_UNICO').agg(
            **{
                'KG MOVIDOS': ('KG MOVIDOS', 'sum'),
                'Tipo de transporte': ('TIPO DE TRANSPORTE', 'first')
            }
        ).reset_index()
        df_eficiencia['Capacidad esperada (kg)'] = df_eficiencia['Tipo de transporte'].astype(str).str.strip().str.upper().map(tipos_capacidad)
        df_eficiencia = df_eficiencia.dropna(subset=['Capacidad esperada (kg)'])
        df_eficiencia['Eficiencia de carga (%)'] = (
            df_eficiencia['KG MOVIDOS'] / df_eficiencia['Capacidad esperada (kg)'] * 100
        )
        if not df_eficiencia.empty:
            fig_eficiencia = px.bar(
                df_eficiencia,
                x='ID_VIAJE_UNICO',
                y='Eficiencia de carga (%)',
                color='Tipo de transporte',
                hover_data=['KG MOVIDOS', 'Capacidad esperada (kg)'],
                title="Grafico de eficiencia"
            )
            fig_eficiencia.update_layout(
                xaxis_title="Índice de viaje",
                yaxis_title="Eficiencia de carga (%)"
            )
...
with tab2:
    ...
    if df_bv.empty:
        ...
        df_b_grouped['Costo por kg'] = np.where(
            df_b_grouped['KG MOVIDOS'] > 0,
            df_b_grouped['FLETE FACTURA'] / df_b_grouped['KG MOVIDOS'],
            0
        )
        df_b_grouped['Ventas vs Flete Factura'] = np.where(
            df_b_grouped['FLETE FACTURA'] > 0,
            df_b_grouped['Facturación'] / df_b_grouped['FLETE FACTURA'] * 100,
            0
        )
        ...
        if not df_show.empty:
            ...
            if 'Ventas vs Flete Factura' in df_vis.columns:
                df_vis['Ventas vs Flete Factura'] = df_vis['Ventas vs Flete Factura'].apply(lambda x: f"{x:.1f}%")
            # Orden columnas
            cols_orden = [c for c in ['ID_VIAJE_UNICO','RUTA','Origen','Destino','Unidad','Tarimas','KG MOVIDOS','Costo por kg','Facturación','FLETE FACTURA','Ventas vs Flete Factura','MANIOBRAS','REPARTOS','DEMORAS Y ESTADIAS','OTROS','TOTAL FLETE','Diferencia vs Media Base','% vs Media'] if c in df_vis.columns]
        ...
        fig_costo_kg = px.bar(
            ...
            y='Costo por kg',
            color='RUTA',
            hover_data=['Unidad', 'KG MOVIDOS', 'FLETE FACTURA'],
            title="Costo por kg"
        )
        fig_costo_kg.update_layout(
            xaxis_title="Índice de viaje",
        ...
        if not df_show_mediana.empty:
            ...
            cols_dinero_mediana = ['Facturación','FLETE FACTURA','Costo por kg','MANIOBRAS','REPARTOS','DEMORAS Y ESTADIAS','OTROS','TOTAL FLETE']
            ...
            if 'Ventas vs Flete Factura' in df_vis_mediana.columns:
                df_vis_mediana['Ventas vs Flete Factura'] = df_vis_mediana['Ventas vs Flete Factura'].apply(
                    lambda x: f"{x:.1f}%"
                )
            cols_orden_mediana = [c for c in [
                'ID_VIAJE_UNICO','RUTA','Origen','Destino','Unidad','Tarimas','KG MOVIDOS',
                'Costo por kg','Facturación','FLETE FACTURA','Ventas vs Flete Factura','MANIOBRAS','REPARTOS','DEMORAS Y ESTADIAS',
