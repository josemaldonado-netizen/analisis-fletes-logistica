...
def cargar_y_procesar_datos():
    # Sustituye la autenticación interactiva de Colab por las credenciales de Streamlit Secrets
    ...
    for nombre_hoja, limite_fila in CONFIG_HOJAS.items():
        try:
            ...
            st.warning(f"Error en {nombre_hoja}: {e}")
    ...
    # 3. Limpieza de montos financieros y volumétricos (TU CÓDIGO EXACTO)
    cols_a_limpiar = [
...
if not df.empty:
    st.success("✅ Base de datos consolidada y validada correctamente.")

    # ---------------------------------------------------------
    # INTERFAZ STREAMLIT CON TUS FUNCIONES
    # ---------------------------------------------------------
    ...
    # TAB 1: Ejecutar tu función comparativa
    with tab1:
    ...
    # TAB 2: Tu auditoría de anomalías
    with tab2:
        ...
        if not viajes_altos.empty:
            ...
            st.warning(f"🚨 Auditoría: {len(viajes_altos)} viajes superan la media de la Semana {sem_a}")
...
3. 💡 Recomendaciones de Negocio"""
    ...
    # TAB 4: Tu simulación de Chat
    with tab4:
        pregunta = st.text_input("Consulta a tus fletes:")
        if pregunta:
            ...
            if "tarima" in query:
                if 'COSTO POR TARIMA' in df.columns and 'TRANSPORTISTA' in df.columns:
                    ...
                    st.success(f"🚛 Transportista más alto: **{agrupado.idxmax()}** con **${agrupado.max():,.2f}** por tarima.")
            elif "demora" in query:
                if 'DEMORAS Y ESTADIAS' in df.columns:
                    df_demoras = df[df['DEMORAS Y ESTADIAS'] > 0]
                    st.warning(f"🚨 **{len(df_demoras)} viajes** con demoras, sumando **${df_demoras['DEMORAS Y ESTADIAS'].sum():,.2f}**.")
            elif "viaje" in query:
