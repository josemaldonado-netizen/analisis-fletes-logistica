...
with tab2:
    ...
    if df_bv.empty:
        ...
        ventas_flete_col1, ventas_flete_col2 = st.columns(2)
        with ventas_flete_col1:
            st.metric(
                f"Ventas vs Flete Factura base — {periodo_base_audit}",
                f"{ventas_vs_flete_a:,.1f}%",
                help="(IMPORTE FACTURADO SIN IVA / FLETE FACTURA) × 100."
            )
        with ventas_flete_col2:
            st.metric(
                f"Ventas vs Flete Factura actual — {periodo_actual_audit}",
                f"{ventas_vs_flete_b:,.1f}%",
                delta=f"{var_ventas_vs_flete:+.1f}% vs base",
                help="(IMPORTE FACTURADO SIN IVA / FLETE FACTURA) × 100."
            )
        st.caption(
