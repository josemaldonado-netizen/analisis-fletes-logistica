...
if st.session_state.get("_firma_archivo_actual") != _firma_archivo:
    st.cache_data.clear()
    st.session_state["_firma_archivo_actual"] = _firma_archivo
    st.info(f"🧹 Caché limpiada — nuevo archivo detectado ({archivo_subido.name}, {len(_bytes_archivo):,} bytes).")
...
# --- DEBUG TEMPORAL: meses detectados en df_raw (SIN filtros aplicados) ---
if 'MES FACTURA' in df_raw.columns:
    st.write("🔍 Debug — Meses en df_raw (sin filtros):", sorted(df_raw['MES FACTURA'].dropna().unique().tolist()))
else:
    st.write("🔍 Debug — La columna 'MES FACTURA' no existe en df_raw.")
...
# --- DEBUG TEMPORAL: meses detectados en df (CON filtros aplicados) ---
if 'MES FACTURA' in df.columns:
    st.write("🔍 Debug — Meses en df (con filtros aplicados):", sorted(df['MES FACTURA'].dropna().unique().tolist()))
    st.write("🔍 Debug — Filtros activos:", {
