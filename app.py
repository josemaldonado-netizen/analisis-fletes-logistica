...
def procesar_archivo(file):
    ...
    # LÓGICA ROBUSTA DE CONSOLIDACIÓN DE EMBARQUES / VIAJES
    id_viajes = []
    es_viaje_principal = []
    ...
    if 'EMBARQUE' in df_out.columns:
        ...
        for idx, row in df_out.iterrows():
            ...
            # Si hay un número de embarque real
            if emb not in ['NA', 'NAN', '', 'NONE', '0']:
                id_viajes.append(f"EMB_{emb}")
                es_viaje_principal.append(True)
            else:
                # Si es NA pero SÍ se cobró flete -> Cada fila es UN VIAJE ÚNICO
                if flete > 0:
                    id_viajes.append(f"VIAJE_NA_INDEX_{idx}")
                    es_viaje_principal.append(True)
                else:
                    # Si es NA y $0 flete -> Factura secundaria
                    id_viajes.append(f"FACTURA_SECUNDARIA_INDEX_{idx}")
    ...
    if not df_raw.empty:
        ...
        if len(periodos) > 0:
            ...
            with tab1:
                ...
                # CÁLCULO DE VIAJES INDEPENDIENTES
