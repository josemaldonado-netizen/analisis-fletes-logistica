...
with tab1:
    ...
    # Mezcla de envío por tipo de transporte
    if 'TIPO DE TRANSPORTE' in df.columns:
        columnas_mezcla = {
            'Kgs': 'KG MOVIDOS',
            'Valor $': 'IMPORTE FACTURADO SIN IVA',
            'Tarimas': 'TARIMAS TOTALES POR VIAJE'
        }
        df_mezcla = df.copy()
        df_mezcla['TIPO DE TRANSPORTE'] = df_mezcla['TIPO DE TRANSPORTE'].fillna('SIN ESPECIFICAR').astype(str).str.strip()
        df_mezcla.loc[df_mezcla['TIPO DE TRANSPORTE'].eq(''), 'TIPO DE TRANSPORTE'] = 'SIN ESPECIFICAR'

        mezcla_conteo = df_mezcla.groupby('TIPO DE TRANSPORTE', dropna=False).size().rename('Conteo')
        mezcla_metricas = pd.DataFrame({'Conteo': mezcla_conteo})
        for etiqueta, columna in columnas_mezcla.items():
            if columna in df_mezcla.columns:
                mezcla_metricas[etiqueta] = df_mezcla.groupby('TIPO DE TRANSPORTE', dropna=False)[columna].sum()
            else:
                mezcla_metricas[etiqueta] = 0

        mezcla_larga = (
            mezcla_metricas.reset_index()
            .melt(
                id_vars='TIPO DE TRANSPORTE',
                value_vars=['Conteo', 'Kgs', 'Valor $', 'Tarimas'],
                var_name='Métrica',
                value_name='Valor'
            )
        )
        if not mezcla_larga.empty:
            fig_mezcla = px.bar(
                mezcla_larga,
                y='Métrica',
                x='Valor',
                ...
                barmode='stack',
                barnorm='percent',
                title='Mezcla de envío',
                category_orders={'Métrica': ['Conteo', 'Kgs', 'Valor $', 'Tarimas']},
                labels={'Valor': 'Participación (%)', 'Métrica': ''}
            )
            fig_mezcla.update_layout(
                xaxis=dict(range=[0, 100], ticksuffix='%'),
                yaxis=dict(categoryorder='array', categoryarray=['Conteo', 'Kgs', 'Valor $', 'Tarimas']),
                legend_title_text='TIPO DE TRANSPORTE'
            )
            fig_mezcla.update_traces(hovertemplate='%{fullData.name}<br>%{y}: %{x:.1f}%<extra></extra>')
