...
MESES_EQUIVALENTES = {
    ...
    'DICIEMBRE': 'DICIEMBRE',
    # Inglés, incluyendo abreviaturas frecuentes de Excel
...
def _normalizar_mes(valor):
    """Convierte nombres de mes en español o inglés al formato español usado por la app."""
    if pd.isna(valor):
        return np.nan
    texto_mes = str(valor).strip().upper().replace('.', '')
    return MESES_EQUIVALENTES.get(texto_mes, texto_mes)
...
def procesar_archivo_cached(file_bytes: bytes, file_name: str):
    ...
    if file_name.lower().endswith('.xlsx'):
        ...
        # Lee hojas de meses en español o inglés; conserva el nombre real de la hoja.
        hojas_a_leer = [h for h in excel_file.sheet_names if _normalizar_mes(h) in NOMBRES_MESES]
        if not hojas_a_leer:
        ...
        for nombre_hoja in hojas_a_leer:
            ...
            # Convierte nombres de hojas en inglés al mes estándar en español.
            mes_hoja = _normalizar_mes(nombre_hoja)
            if mes_hoja in NOMBRES_MESES:
                df_temp['MES_ORIGEN'] = mes_hoja
            else:
    ...
    if not col_mes_fact:
        ...
        # Normaliza meses de Excel en español o inglés para que el filtro funcione
        # sin depender del idioma configurado en la computadora.
...
# ---------- TABS ----------
tab1, tab2, tab3, tab4 = st.tabs(["📊 Comparativo Financiero y Gráficos","🚨 Auditoría de Anomalías","📝 Prompt para IA Executive","🗂️ Datos Generales"])
...
with tab1:
    ...
    df_mezcla_base = pd.concat([df_a, df_b]).loc[lambda d: ~d.index.duplicated()]
    if 'TIPO DE TRANSPORTE' in df_mezcla_base.columns:
        if 'ID_VIAJE_UNICO' in df_mezcla_base.columns:
            conteo_mezcla = df_mezcla_base.drop_duplicates('ID_VIAJE_UNICO').groupby('TIPO DE TRANSPORTE')['ID_VIAJE_UNICO'].nunique()
        else:
            conteo_mezcla = df_mezcla_base['TIPO DE TRANSPORTE'].value_counts()
        kg_mezcla = df_mezcla_base.groupby('TIPO DE TRANSPORTE')['KG MOVIDOS'].sum() if 'KG MOVIDOS' in df_mezcla_base.columns else pd.Series(dtype=float)
        valor_mezcla = df_mezcla_base.groupby('TIPO DE TRANSPORTE')['IMPORTE FACTURADO SIN IVA'].sum() if 'IMPORTE FACTURADO SIN IVA' in df_mezcla_base.columns else pd.Series(dtype=float)
        tarimas_mezcla = df_mezcla_base.groupby('TIPO DE TRANSPORTE')['TARIMAS TOTALES POR VIAJE'].sum() if 'TARIMAS TOTALES POR VIAJE' in df_mezcla_base.columns else pd.Series(dtype=float)
        ...
        for tipo in tipos_mezcla:
            valores_tipo = [
                conteo_mezcla.get(tipo, 0),
                kg_mezcla.get(tipo, 0),
...
with tab2:
    ...
    if df_bv.empty:
            ...
            if not viajes_eficiencia_baja.empty:
                st.dataframe(
                    viajes_eficiencia_baja[[col for col in [
                        'ID_VIAJE_UNICO', 'FAC', 'Destino',
                        'Método de agrupación', 'Unidad', 'RUTA', 'Tarimas', 'KG MOVIDOS',
        ...
        if not viajes_tr2_flete_alto.empty:
            columnas_tr2 = [
                col for col in [
                    'ID_VIAJE_UNICO', 'FAC', 'RUTA', 'Unidad', 'Tipo de Embarque',
                    'Facturación', 'FLETE FACTURA', '% Flete sobre Facturación'
                ] if col in viajes_tr2_flete_alto.columns
            ]
            df_tr2_vis = viajes_tr2_flete_alto[columnas_tr2].copy()
            for col in ['Facturación', 'FLETE FACTURA']:
                if col in df_tr2_vis.columns:
                    df_tr2_vis[col] = df_tr2_vis[col].apply(
...
with tab4:
    st.subheader("🗂️ Datos Generales")
    st.caption(
        "Esta sección utiliza el 100% de la base de datos cargada, sin considerar los filtros "
        "operativos del panel lateral ni los periodos de comparación seleccionados."
    )

    # --- Mezcla de envío (sin filtros) ---
    st.markdown("---")
    st.markdown("### 🚚 Mezcla de envío")
    df_mezcla_gen = df_raw.copy()
    if 'TIPO DE TRANSPORTE' in df_mezcla_gen.columns:
        if 'ID_VIAJE_UNICO' in df_mezcla_gen.columns:
            conteo_mezcla_gen = df_mezcla_gen.drop_duplicates('ID_VIAJE_UNICO').groupby('TIPO DE TRANSPORTE')['ID_VIAJE_UNICO'].nunique()
        else:
            conteo_mezcla_gen = df_mezcla_gen['TIPO DE TRANSPORTE'].value_counts()
        kg_mezcla_gen = df_mezcla_gen.groupby('TIPO DE TRANSPORTE')['KG MOVIDOS'].sum() if 'KG MOVIDOS' in df_mezcla_gen.columns else pd.Series(dtype=float)
        valor_mezcla_gen = df_mezcla_gen.groupby('TIPO DE TRANSPORTE')['IMPORTE FACTURADO SIN IVA'].sum() if 'IMPORTE FACTURADO SIN IVA' in df_mezcla_gen.columns else pd.Series(dtype=float)
        tarimas_mezcla_gen = df_mezcla_gen.groupby('TIPO DE TRANSPORTE')['TARIMAS TOTALES POR VIAJE'].sum() if 'TARIMAS TOTALES POR VIAJE' in df_mezcla_gen.columns else pd.Series(dtype=float)

        tipos_mezcla_gen = sorted(set(conteo_mezcla_gen.index) | set(kg_mezcla_gen.index) | set(valor_mezcla_gen.index) | set(tarimas_mezcla_gen.index))
        categorias_mezcla_gen = ['Conteo', 'Kgs', 'Valor $', 'Tarimas']
        formatos_mezcla_gen = {'Conteo': lambda v: f"{v:,.0f}", 'Kgs': lambda v: f"{v:,.0f}", 'Valor $': lambda v: f"${v:,.0f}", 'Tarimas': lambda v: f"{v:,.0f}"}

        fig_mezcla_gen = go.Figure()
        for tipo in tipos_mezcla_gen:
            valores_tipo_gen = [
                conteo_mezcla_gen.get(tipo, 0),
                kg_mezcla_gen.get(tipo, 0),
                valor_mezcla_gen.get(tipo, 0),
                tarimas_mezcla_gen.get(tipo, 0),
            ]
            textos_tipo_gen = [formatos_mezcla_gen[cat](v) for cat, v in zip(categorias_mezcla_gen, valores_tipo_gen)]
            fig_mezcla_gen.add_trace(go.Bar(y=categorias_mezcla_gen, x=valores_tipo_gen, name=str(tipo), orientation='h', text=textos_tipo_gen, textposition='inside'))
        fig_mezcla_gen.update_layout(title="Mezcla de Envío — Vista Anual (100% de la Base)", barmode='stack', barnorm='percent', height=340, margin=dict(t=50,b=20), legend=dict(orientation="h", y=1.08))
        fig_mezcla_gen.update_xaxes(showticklabels=False, title=None)
        st.plotly_chart(fig_mezcla_gen, use_container_width=True)
    else:
        st.info("No se encontró la columna 'TIPO DE TRANSPORTE' en la base de datos.")

    # --- Costo por KG por mes y tipo de transporte (sin filtros, en unidades $) ---
    st.markdown("---")
    st.markdown("### 📦 Costo por KG — Vista Anual por Tipo de Transporte")
    st.caption(
        "Costo por KG = FLETE FACTURA / KG MOVIDOS, agrupado por viaje único (INDICE VIAJES) "
        "y desglosado por mes y Tipo de Transporte. Valores en unidades de $, no en porcentajes."
    )
    cols_necesarias_gen = {'ID_VIAJE_UNICO', 'TIPO DE TRANSPORTE', 'MES FACTURA', 'FLETE FACTURA', 'KG MOVIDOS'}
    if cols_necesarias_gen.issubset(set(df_raw.columns)):
        # 1) Agrupar a nivel de viaje único (INDICE VIAJES) para no duplicar filas de un mismo viaje.
        df_viaje_gen = df_raw.groupby('ID_VIAJE_UNICO').agg(
            FLETE_FACTURA=('FLETE FACTURA', 'sum'),
            KG_MOVIDOS=('KG MOVIDOS', 'sum'),
            MES_FACTURA=('MES FACTURA', 'first'),
            TIPO_TRANSPORTE=('TIPO DE TRANSPORTE', 'first'),
        ).reset_index()

        # 2) Agrupar por Mes + Tipo de Transporte y calcular el costo por KG.
        df_costo_mes_gen = df_viaje_gen.groupby(['MES_FACTURA', 'TIPO_TRANSPORTE']).agg(
            FLETE_FACTURA=('FLETE_FACTURA', 'sum'),
            KG_MOVIDOS=('KG_MOVIDOS', 'sum'),
        ).reset_index()
        df_costo_mes_gen['COSTO_KG'] = np.where(
            df_costo_mes_gen['KG_MOVIDOS'] > 0,
            df_costo_mes_gen['FLETE_FACTURA'] / df_costo_mes_gen['KG_MOVIDOS'],
            0
        )
        df_costo_mes_gen = df_costo_mes_gen[df_costo_mes_gen['MES_FACTURA'].isin(NOMBRES_MESES)]

        if df_costo_mes_gen.empty:
            st.info("No hay datos suficientes de MES FACTURA para construir esta vista.")
        else:
            meses_presentes_gen = [m for m in NOMBRES_MESES if m in df_costo_mes_gen['MES_FACTURA'].unique()]
            tipos_costo_gen = sorted(df_costo_mes_gen['TIPO_TRANSPORTE'].dropna().unique())

            fig_costo_mes_gen = go.Figure()
            for tipo in tipos_costo_gen:
                sub_tipo = df_costo_mes_gen[df_costo_mes_gen['TIPO_TRANSPORTE'] == tipo].set_index('MES_FACTURA')
                sub_tipo = sub_tipo.reindex(meses_presentes_gen)
                y_vals = sub_tipo['COSTO_KG'].fillna(0)
                # Barras apiladas por mes (una serie por Tipo de Transporte)
                fig_costo_mes_gen.add_trace(go.Bar(
                    x=meses_presentes_gen,
                    y=y_vals,
                    name=str(tipo),
                    text=[f"${v:,.2f}" if v > 0 else "" for v in y_vals],
                    textposition='inside',
                ))
                # Línea por Tipo de Transporte superpuesta a su propio segmento
                fig_costo_mes_gen.add_trace(go.Scatter(
                    x=meses_presentes_gen,
                    y=y_vals,
                    name=f"{tipo} (tendencia)",
                    mode='lines+markers',
                    line=dict(width=2, dash='dot'),
                    marker=dict(size=6),
                    showlegend=False,
                    hovertemplate=f"{tipo}: $%{{y:,.2f}}/kg<extra></extra>",
                ))

            fig_costo_mes_gen.update_layout(
                title="Datos General - Anual — Costo por KG (mensual, por Tipo de Transporte)",
                barmode='stack',
                yaxis_title="Costo por KG ($)",
                xaxis_title="Mes",
                height=420,
            ...
            st.plotly_chart(fig_costo_mes_gen, use_container_width=True)

            with st.expander("📋 Ver tabla de datos (Costo por KG por mes y tipo de transporte)"):
                df_tabla_costo_gen = df_costo_mes_gen.copy()
                df_tabla_costo_gen['MES_FACTURA'] = pd.Categorical(df_tabla_costo_gen['MES_FACTURA'], categories=NOMBRES_MESES, ordered=True)
                df_tabla_costo_gen = df_tabla_costo_gen.sort_values(['MES_FACTURA', 'TIPO_TRANSPORTE'])
                df_tabla_costo_gen['COSTO_KG'] = df_tabla_costo_gen['COSTO_KG'].apply(lambda v: f"${v:,.2f}")
                df_tabla_costo_gen['FLETE_FACTURA'] = df_tabla_costo_gen['FLETE_FACTURA'].apply(lambda v: f"${v:,.2f}")
                df_tabla_costo_gen['KG_MOVIDOS'] = df_tabla_costo_gen['KG_MOVIDOS'].apply(lambda v: f"{v:,.0f}")
                df_tabla_costo_gen = df_tabla_costo_gen.rename(columns={
                    'MES_FACTURA': 'Mes', 'TIPO_TRANSPORTE': 'Tipo de Transporte',
                    'FLETE_FACTURA': 'Flete Factura', 'KG_MOVIDOS': 'KG Movidos', 'COSTO_KG': 'Costo por KG'
                })
                st.dataframe(df_tabla_costo_gen, use_container_width=True, hide_index=True)
    else:
        faltantes_gen = cols_necesarias_gen - set(df_raw.columns)
        st.warning(f"⚠️ Faltan columnas para construir esta vista: {', '.join(sorted(faltantes_gen))}")
