...
if archivo_subido is not None:
    ...
    if not df_raw.empty:
        ...
        if len(periodos) > 0:
            ...
            with tab1:
                st.subheader(f"📊 Comparativa: {modo_periodo} {per_a} vs {modo_periodo} {per_b}")

                # Resumen de Métricas Clave / Medias
                st.markdown("### 📐 Métricas Promedio y Volumétricas")
                ...
                m_col1.metric("Total Viajes", f"{viajes_b}", delta=f"{viajes_b - viajes_a} vs {modo_periodo} {per_a}")
                m_col2.metric("Costo Medio / Viaje", f"${media_viaje_b:,.2f}", delta=f"{((media_viaje_b - media_viaje_a)/media_viaje_a*100) if media_viaje_a > 0 else 0:+.1f}%")
                m_col3.metric("Costo Medio / Tarima", f"${costo_tarima_b:,.2f}", delta=f"{((costo_tarima_b - costo_tarima_a)/costo_tarima_a*100) if costo_tarima_a > 0 else 0:+.1f}%")
                m_col4.metric("Costo Medio / KM", f"${costo_km_b:,.2f}", delta=f"{((costo_km_b - costo_km_a)/costo_km_a*100) if costo_km_a > 0 else 0:+.1f}%")
